...
from move_engine import get_random_move, get_ai_move

dark_block = pygame.Surface((75, 75))
dark_block.fill((0, 0, 0))  # Black square

light_block = pygame.Surface((75, 75))
light_block.fill((255, 255, 255))  # White square)
...
def draw_background(chess_board):
    ...
    for i in range(8):
        for j in range(8):
            if isinstance(chess_board[i][j], ChessPiece):
                obj = globals()[f'{chess_board[i][j].color}{chess_board[i][j].type}']
...
def start(chess_board):
    ...
    game_over = False
    piece = None
    if chess_board.game_mode == 1 and chess_board.ai:
        get_ai_move(chess_board)
        draw_background(chess_board)
    while running:
        if game_over:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
                ...
                if isinstance(chess_board[x][y], ChessPiece) and (chess_board.get_player_color() == chess_board[x][y].color or not chess_board.ai) and (x, y) not in possible_piece_moves:
                    piece = chess_board[x][y]
                    moves = piece.filter_moves(piece.get_moves(chess_board), chess_board)
                    move_positions = []
                    possible_piece_moves = []
                    for move in moves:
                    ...
                    try:
                        if clicked_move in possible_piece_moves:
                            chess_board.make_move(piece, x, y)
                        ...
                        if chess_board.white_won():
                            game_over = True
                            game_over_txt = 'WHITE WINS!'
                        elif chess_board.black_won():
                            game_over = True
                            game_over_txt = 'BLACK WINS!'
                        elif chess_board.draw():
                            game_over = True
                            game_over_txt = 'DRAW!'
